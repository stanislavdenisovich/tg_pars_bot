...
def handle_idea(msg):
    ...
    if len(idea) < 10 or idea.isdigit() or not any(c.isalpha() for c in idea):
        bot.send_message(
            msg.chat.id,
            "❌ Это не похоже на идею стартапа.\n\n"
            "Пожалуйста, опиши идею подробнее — в 1–2 предложениях:\n"
            "Что вы создаёте, для кого и какую проблему решаете 💡"
        )
        return
...
"""
        ...
        # 6️⃣ Обработка ошибок API
        error_text = str(e)
        if "401" in error_text or "account_deactivated" in error_text:
            bot.send_message(
                msg.chat.id,
                "❌ Ошибка модели: ключ OpenAI недействителен или аккаунт деактивирован.\n\n"
                "Проверь настройки Railway и переменную <code>OPENAI_API_KEY</code> 🔑"
            )
        else:
            bot.send_message(
                msg.chat.id,
                "❌ Это не похоже на идею стартапа.\n"
                "Пожалуйста, опиши свою идею так, чтобы было понятно, что вы делаете и для кого.\n\n"
                "Например:\n"
                "«Онлайн-платформа, которая соединяет студентов и репетиторов по ИТ в Казахстане»"
            )
